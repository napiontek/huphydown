# huphydown <img src="/inst/rmarkdown/templates/thesis/skeleton/figure/HU_Logo_small.png" align="right" width="128px"/>
The `huphydown` package is an adaptation of the popular `thesisdown* package for writing a thesis at the Physics Institute of Humboldt-University of Berlin. The template follows that guidelines of the institute [here](https://fakultaeten.hu-berlin.de/de/mnf/lehre_studium/pruefungsbuero/pruefungsbuero_physik/merkblatt-abschlussarbeit-physik-1.pdf) and can be adjusted as needed in the 'template.tex' file.
...
Under the hood, the HU Physics LaTeX template is used to help documents conform to submission standards, feel free to adjust those to changes in standards and personal preference. Composition and formatting can be done using lightweight [markdown](https://rmarkdown.rstudio.com/authoring_basics.html) syntax, and **R** code and its output can be seamlessly included using [rmarkdown](https://rmarkdown.rstudio.com).
...
Using **huphydown** has some prerequisites which are described below. To compile PDF documents using **R**, you are going to need to have LaTeX installed. By far the easiest way to install LaTeX on any platform is with the [tinytex](https://yihui.name/tinytex/) R package:
...
To use **huphydown** from [RStudio](https://www.rstudio.com/products/rstudio/download/):
...
2) Install the **bookdown** and **huphydown** packages. Note that **huphydown** is not available on CRAN at the moment and that's why `install.packages("huphydown")` won't work. Use `remotes::install_github()` as shown below instead to install the package.
...
3) Use the **New R Markdown** dialog to select **HU Physics Thesis**:
...
### `01-intro.Rmd`, `02-theory.Rmd`, etc.
